...
The bear-off tray (`.off`) caps how many checkers it actually renders at `MAX_VISIBLE_OFF` (`script.js`), independent of the true count in `state.off` — past the cap, a borne-off checker is simply never appended to the DOM rather than appended and visually clipped, so it "disappears" instead of overflowing the tray. `.off-count` (`renderOffCounts`) is the only place the real count is shown, and reads `state.off` directly rather than counting DOM children, since the DOM count is deliberately capped and would undercount past `MAX_VISIBLE_OFF`.
