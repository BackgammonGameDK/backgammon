...
**And when it does appear, it never prints the room code.** It used to lead with `Room ABCDEF · `, which is the one part of that line nobody arriving through Find an opponent can do anything with — the code is already in the address bar, and Copy link and QR both hand it over without anyone reading it off the screen. What is left is only what the player can act on: who they are, and what the game is waiting for. Anyone who genuinely needs the code (reading it aloud to someone in the room) can still see it in the URL. The line got shorter, never taller, so no `--chrome` value moves; it still wraps to two rows at phone widths, which is the state `--chrome`/`--online-row` are measured against — checked by measuring `#room-status` at 1280x800, 900x700 and 320-500px wide, old text against new.
