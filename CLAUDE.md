...
The app opens on an explicit choice — **Find an opponent** (the only filled button, since matchmaking is the ordinary route in), Play on this device, *Rejoin your last game* when there is one, a room-code field folded behind a quiet toggle, and `#version-marker`, which prints the build this device is running (see Running / previewing above) — rather than dropping straight into a hot-seat game with online play as one more button beside Roll and Hints. Built as a full-viewport overlay, the same shape `#celebration` uses, so it sits on top of the finished layout instead of displacing it and therefore needs no space in `--chrome`. The board renders behind it from `idleState()`: the standard opening position with `dice` and `openingRoll` cleared, purely a backdrop.
...
**That read happens at load, which is not the only way a room code arrives (`hashchange`).** Pasting an invite link into the address bar of a tab *already* on the game changes only the fragment, so the browser performs a same-document navigation: no reload, no script re-run, and nothing whatsoever happened — a page that just sits there, indistinguishable from a broken link. The Back button after joining was the same gap from the other side, since joining pushes a history entry by assigning `location.hash`. A `hashchange` listener now covers both, and `roomCodeInUrl()` is the one place the fragment is parsed, shared with the load-time read.

Three things it has to get right, and does. **A code we are already in is ignored** — this client's own hash writes (Find an opponent, Rejoin, join-by-code) all land here a moment after `startOnline` has set `currentRoomCode`, and re-joining on them would tear down the room this tab just entered. **A fragment with no room code exits, but only from an online game**: the URL has stopped pointing at a room, so leaving is what it says, while a hot-seat game is not something an unrelated fragment has any business destroying. **Switching rooms tears the old one down first** via `exitToStartScreen`, which announces the departure — and clears the fragment on its way out, so the new code goes back with `replaceState` rather than an assignment, an assignment being what would re-enter the listener.
...
**And when it does appear, it says as little as it can get away with.** It used to lead with `Room ABCDEF · `, which is the one part of that line nobody arriving through Find an opponent can do anything with — the code is already in the address bar, and Copy link and QR both hand it over without anyone reading it off the screen. **The waiting state then went further and is now the whole line, `Waiting for an opponent…`** — no colour and no queue count. Before an opponent exists your colour decides nothing, and how many other people are searching is a fact about the queue rather than something the player can act on; the one thing an empty queue actually calls for is Copy link, which sits right beside it. The other states keep the colour, since by then it is the only place the app names which side you are playing: `You are White — opponent disconnected`, `— opponent left the game`, or a bare `Spectating`. What is left everywhere is only what the player can act on. Anyone who genuinely needs the code (reading it aloud to someone in the room) can still see it in the URL. Every line got shorter, never taller, so no `--chrome` value moves. The tallest state is still two rows at phone widths — the disconnected/left lines wrap at 390px and below, which is what `--chrome`/`--online-row` are measured against — while the waiting line now fits on one row at every width. Checked by measuring `#room-status` at 1280x800, 900x700 and 320-500px wide, every line against the old text.

**`renderRoomStatus` builds the line whole rather than as a prefix plus a suffix**, because the waiting case replaces the prefix rather than adding to it. `worthARow` then falls out of the line being non-empty, spectators included, instead of naming them separately.
...
**The turn indicator says "Game over" once won**, rather than continuing to claim a turn next to the result ("White wins!" in hot-seat, "You win!" or "Opponent wins" online — see `winnerLine`). It says something rather than emptying because `.status-row`'s height is part of the board's hand-measured budget.

**Whose turn it is is a colour before it is a sentence (`setTurnIndicator`, `turnIndicatorColor`, `.turn-dot`).** The line used to be one run of same-sized bold text — "White's turn — opening roll: White 6, Black 5" — in which the two words that matter are the two easiest to skim past, and that run is the *first* thing a new game shows. Reported as hard to read at exactly that moment. So the indicator is now built from three parts rather than one string: a checker-styled dot in the colour being waited on, the turn itself, and the opening-roll recap demoted to a smaller, dimmer `.turn-note` (it is a recap of the two dice sitting right beside it, so it should not compete with the turn at equal weight). The dot is the same wordless cue the per-player dice colours already carry, and the same pure-luminance distinction, so it survives any colour vision deficiency.

`turnIndicatorColor()` returns null when it is nobody's turn — a finished game, an unbroken tie, a spectator — and during the opening returns whoever still owes a die, which online is always *you*, since the line speaks to you directly there ("Roll to see who starts"). During the opening it is therefore also a quiet answer to "which colour am I?", which the room row stopped saying when the waiting line was cut down; **once play starts it is not** — it names the player on turn, whoever is reading.

**Which is why the words have to do the rest (`turnLine`, `winnerLine`).** Both clients render the same shared state, so "White's turn" is identical on both screens and answers the wrong question: it says which colour is on turn, not whether that is you. The colour dot alone did not fix it and could not — it is the same dot on both devices. Reported straight from a two-device test: *"it says White's turn on both devices, the white dot is also on both devices."* So a seated online client reads **"Your turn"** or **"Opponent's turn"**, and at the end **"You win!"** or **"Opponent wins"** (`#game-over` and the celebration overlay share `winnerLine`). Hot-seat keeps the colour names — two people at one screen have no "you" to address — and so do spectators, who are neither player.

The standing lesson, worth applying to any player-facing text added later: **check what it says on the *other* device before calling it done.** A cue that reads the same on both screens cannot distinguish them, however clear it is in isolation. `titleCue` already had this right; the visible line did not.

**`.turn-dot` is sized in `em` and deliberately smaller than the line box**, because `.status-row`'s height is part of `--chrome` and a dot that decided the row's height would silently shrink the board. Same for `.turn-note` being smaller than the text it follows. Verified rather than assumed: the status row and the board's rect measure identical before and after at 1280x800, 390x844 and 812x375. `setTurnIndicator` also appends a real space text node before the note, so the element's `textContent` — what a screen reader announces and what a copy of the line contains — does not read "White's turnopening roll: …".
...
- **Everything else is refused only for a while.** A client that has fallen behind — a reconnect delivers only the latest state — legitimately sees jumps it cannot account for, and refusing forever would wedge the game permanently. That is a worse and far likelier harm than an unpunished cheat, so after three *distinct* refused states (`REJECTIONS_BEFORE_RESYNC`, in `rules.js` beside the policy that reads it) it accepts and resynchronises. Distinct rather than a plain count on purpose: a client that is behind watches the game move on, so what it refuses keeps changing, whereas the same board arriving again is far likelier to be someone leaning on it. Resending one rejected board therefore gets nowhere.
...
**The waiting line no longer shows how many others are searching.** It said "nobody else is searching" or "2 others searching", which was honest about the queue but not something a waiting player could do anything with — the useful move when nobody is queuing is Copy link, which is beside the text either way. `script.js` therefore no longer subscribes to the count; `watchLobbyCount` stays in `sync.js` because the test suite observes the queue through it, and it is one call away if the count ever earns its place back.
...
`--chrome` is measured against the *tallest* state the layout takes, which is an online game: `.online-area` then holds `#room-status`, uncollapsed, with its longest status text. Measuring against the tallest is what guarantees the board can never overrun. A hot-seat game has no room status, and neither does an online game whose row has nothing to report, so `body:not(.showing-room-row) .online-area` is hidden outright — removing its flex gap too, not merely its content — and `--online-row` is how much height that frees, measured per breakpoint the same by-hand way, as the difference in `.board`'s top offset between the two states. `--reclaimed` is what the calc actually spends: `0px` while the row is showing, and `var(--online-row)` on a `body` without the `showing-room-row` class that `renderRoomStatus` sets. So the board claims that height whenever the row is quiet (+57px of board width at the ≥901px breakpoint) and gives it straight back the moment there is news. *(This sentence used to describe an `.online` class added by `startOnline()`; the keying moved to `showing-room-row` when the row started earning its space, and there is no `.online` class any more.)* The check that the value is right is that the board's *bottom edge* lands in the same place in both modes — too large a value shows up as the board overrunning it.
...
- **`cache-bust-check`** — two gates. It compares which of the eleven versioned assets have uncommitted changes against whether the `?v=N` tags were touched too, exiting non-zero if a bump was missed; and, on every run including a clean tree, it checks that every `?v=` in `index.html` and `manifest.json` carries the *same* number, naming the offending line if not. That second gate used to be a line of prose asking for a visual check, which is precisely how `style.css` once ended up a version behind the script tags.
